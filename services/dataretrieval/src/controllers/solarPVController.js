...
https://developer.nrel.gov/api/pvwatts/v8.json?api_key=DEMO_KEY&lat=30&lon=69&azimuth=180&system_capacity=4&losses=14&array_type=1&module_type=0&gcr=0.4&dc_ac_ratio=1.2&inv_eff=96.0&radius=0&dataset=nsrdb&tilt=10&&soiling=12|4|45|23|9|99|67|12.54|54|9|0|7.6&albedo=0.3&bifaciality=0.7
    ...
    // Month June
    solarpvdata.outputs.ac_monthly[5] =
      solarpvdata.outputs.ac_monthly[4] + solarpvdata.outputs.ac_monthly[5];

    // Month July
    solarpvdata.outputs.ac_monthly[6] =
      solarpvdata.outputs.ac_monthly[6] + solarpvdata.outputs.ac_monthly[7] / 2;

    // DC Adjustment
    // Month June
    solarpvdata.outputs.dc_monthly[5] =
      solarpvdata.outputs.dc_monthly[4] + solarpvdata.outputs.dc_monthly[5];

    // Month July
    solarpvdata.outputs.dc_monthly[6] =
      solarpvdata.outputs.dc_monthly[6] + solarpvdata.outputs.dc_monthly[7] / 2;

    // POA Adjustment
    // Month June
    solarpvdata.outputs.poa_monthly[5] =
      solarpvdata.outputs.poa_monthly[4] + solarpvdata.outputs.poa_monthly[5];

    // Month July
    solarpvdata.outputs.poa_monthly[6] =
      solarpvdata.outputs.poa_monthly[6] +
      solarpvdata.outputs.poa_monthly[7] / 2;

    // Sol Rad Adjustment
    // Month June
    solarpvdata.outputs.solrad_monthly[5] =
      solarpvdata.outputs.solrad_monthly[4] +
      solarpvdata.outputs.solrad_monthly[5];

    // Month July
    solarpvdata.outputs.solrad_monthly[6] =
      solarpvdata.outputs.solrad_monthly[6] +
      solarpvdata.outputs.solrad_monthly[7] / 2;

    console.log(solarpvdata.outputs.ac_monthly[5]);
    console.log(solarpvdata.outputs.ac_monthly[6]);

    console.log(solarpvdata.outputs);
